...
class UsuarioPersonalizado(AbstractBaseUser, PermissionsMixin):
    ...
    def desactivar_credencial(self):
        self.credencial.desactivar()

    def activar_credencial(self):
        self.credencial.activar()

    def get_estado_solicitud(self):
        ultima_solicitud: SolicitudCredencial = self.solicitudes.order_by('-fecha_solicitud').first()
        if ultima_solicitud is None:
            return None
        return ultima_solicitud.estado

    def get_fecha_solicitud(self) -> date | None:
        ultima_solicitud: SolicitudCredencial = self.solicitudes.order_by('-fecha_solicitud').first()
        if ultima_solicitud is None:
            return None
        return ultima_solicitud.fecha_solicitud

    def get_tipo_solicitud(self) -> str | None:
        ultima_solicitud: SolicitudCredencial = self.solicitudes.order_by('-fecha_solicitud').first()
        if ultima_solicitud is None:
            return None
        return ultima_solicitud.tipo_solicitud

    def get_tipo_sangre(self) -> str | None:
        if self.ficha_medica_existe():
            return self.ficha_medica.tipo_sangre
        return None

    def get_alergias(self) -> str | None:
        if self.ficha_medica_existe():
            return self.ficha_medica.alergias
        return None

    def get_enfermedades_cronicas(self) -> str | None:
        if self.ficha_medica_existe():
            return self.ficha_medica.enfermedades_cronicas
        return None

    def get_nombre_contacto_emergencia(self) -> str | None:
        if self.contacto_emergencia_existe():
            return self.contacto_emergencia.nombre_contacto_emergencia
        return None

    def get_numero_contacto_emergencia(self) -> str | None:
        if self.contacto_emergencia_existe():
            return self.contacto_emergencia.numero_contacto_emergencia
        return None
    ...
    def get_estado_credencial(self):
        if self.credencial is None:
            return None
        return self.credencial.estado_credencial

    def get_clave_credencial(self):
        if self.credencial is None:
            return None
        return self.credencial.clave_credencial
